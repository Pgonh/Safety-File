...
Database SQLite sẽ được khởi tạo tự động khi chạy server lần đầu tiên. Tuy nhiên, bạn có thể cấu hình đường dẫn database trong file `backend/config/database.js`:

```javascript
// backend/config/database.js
const DATABASE_PATH = "./data/safety_file.db";
```

Thư mục `data/` sẽ được tạo tự động nếu chưa tồn tại.

### 5. Cài Đặt Settings.json
